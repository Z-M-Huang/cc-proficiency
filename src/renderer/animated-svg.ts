...
import {
  ...
  escapeXml, confidenceSymbol, formatHours,
  svgDefs, miniBarColor, miniBarOpacity, renderCalibratingBadge,
  buildLocaleEntries, switchedText,
} from "./svg.js";
import type { LocaleEntry } from "./svg.js";
...
function switchedAnimatedText(
  ...
  for (const e of entries) {
    ...
    if (e.lang) {
      lines.push(`<text systemLanguage="${e.lang}" ${attrs}>${text}${animate}</text>`);
    } else {
      lines.push(`<text ${attrs}>${text}${animate}</text>`);
    }
  }
  lines.push("</switch>");
  return lines.join("");
...
export function renderAnimatedFullBadge(result: ProficiencyResult, tokenWindows?: TokenWindows): string {
  ...
  const domainSvg = result.domains
    .map((d, i) => renderAnimatedDomainRow(d, 62 + i * 28, i, entries))
    .join("\n");

  const earlyAnimate = `<animate attributeName="opacity" from="0" to="1" dur="${FADE_DUR}" begin="${footerDelay}s" fill="freeze"/>`;
  const phaseLabel = result.phase === "early"
    ? switchedAnimatedText(`x="${width - 25}" y="${footerY + 16}" fill="#d29922" font-size="10" font-family="${MONO}" text-anchor="end" opacity="0"`, (b) => b.earlyResults, entries, earlyAnimate)
    : "";
  ...
  const footerAttrs = `x="25" y="${footerY + 16}" fill="${C.textMuted}" font-size="11" font-family="${MONO}" opacity="0"`;
  const footerSvg = switchedAnimatedText(footerAttrs, (b) => `${formatHours(result.features.totalHours)} \u00B7 ${result.sessionCount} ${b.sessions} \u00B7 ${result.projectCount} ${b.projects}${streakPart}${achievePart}`, entries, footerAnimate);

  const tokenAnimate = `<animate attributeName="opacity" from="0" to="1" dur="0.4s" begin="${footerDelay}s" fill="freeze"/>`;
  ...
  const tokenSvg = showTokens
    ? switchedAnimatedText(`x="25" y="${footerY + 30}" fill="${C.textDim}" font-size="10" font-family="${MONO}" opacity="0"`, (b) => b.tokensPrefix + tokenNums, entries, tokenAnimate)
    : "";
  ...
  ${renderAnimatedMiniBarGrid(result.features.featureScores, miniBarY, miniDelay, entries)}
 ...
 */
export function renderAnimatedBadge(result: ProficiencyResult, tokenWindows?: TokenWindows): string {
  if (result.phase === "calibrating") {
    return renderCalibratingBadge(result, tokenWindows);
  }
  return renderAnimatedFullBadge(result, tokenWindows);
}
